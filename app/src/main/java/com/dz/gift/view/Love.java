...
public class Love  extends SurfaceView implements SurfaceHolder.Callback,Runnable
{
    ...
    {
        // TODO Auto-generated method stub
//        setBackgroundColor(getResources().getColor(R.color.normal_bg));
    ...
    private void draw()
    {
        ...
        try
        {
            ...
            for (i = 0; i <= 90; i++) {
                ...
                for (j = 0; j <= 90; j++) {
                    r = Math.PI / 45 * i * (1 - Math.sin(Math.PI / 45 * j)) * 60;
 
                    x = r * Math.cos(Math.PI / 45 * j) * Math.sin(Math.PI / 45 * i) + getMeasuredWidth() / 2;
 
                    y = -r * Math.sin(Math.PI / 45 * j) + getMeasuredHeight() / 4;
